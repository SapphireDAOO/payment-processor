...
interface INotes {
    ...
    event NoteCreated(
        uint216 indexed orderId, uint256 indexed noteId, address indexed author, bool share, bytes encryptedContent
    );

    /// @notice Emitted when a user updates their opened state for a note.
    event NoteStateChanged(uint216 indexed orderId, uint256 indexed noteId, address indexed user, bool opened);
     ...
     */
    function setOpened(uint216 orderId, uint256 noteId, bool open) external;
     ...
     */
    function getNoteCount(uint216 orderId) external view returns (uint256 total);
     ...
     */
    function isOpened(uint216 orderId, uint256 noteId, address user) external view returns (bool isOpen);
