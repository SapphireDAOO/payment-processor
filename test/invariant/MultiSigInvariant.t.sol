...
contract MultiSigInvariant is StdInvariant, MultiSigSetUp {
    ...
    function invariant_nonceConsistency() external view {
        assertEq(multisig.getNonce(), handler.ghostNonce());
    }
    ...
    function invariant_validTransactionStatus() external view {
        ...
        for (uint256 i; i < count; i++) {
            ...
            assertTrue(status == PENDING || status == APPROVED || status == EXECUTED);
        }
    }

    function invariant_signerCountConsistency() external view {
        assertEq(handler.getSignerCount(), multisig.getSignerCount());
    }

    function invariant_thresholdConsistency() external view {
        assertEq(handler.getGhostThreshold(), multisig.getThreshold());
    }
}
